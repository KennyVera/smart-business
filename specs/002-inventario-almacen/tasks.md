# 002 — Tareas: inventario y almacén

Leyenda: `[x]` COMPLETADO · `[ ]` PENDIENTE

---

## Completado

- [x] CRUD de productos (SKU, nombre, categoría, costo, precio, `aplica_iva`).
- [x] CRUD de proveedores + FK en producto + WhatsApp desde Kardex.
- [x] Renderizado de imágenes locales (`ImageField` + `/media/productos/`).
- [x] Kardex en Offcanvas (historial de movimientos por producto/sucursal).
- [x] Reportes operativos en PDF.
- [x] Stock por sucursal, alertas de mínimo, lotes/caducidad y mermas
      (flujos ya presentes en el módulo de inventario).

## Pendiente

- [ ] Importación masiva por **CSV** (catálogo y/o stock inicial).
- [ ] Exportación masiva por **CSV** (mismo contrato, para intercambio con
      proveedores o planillas de conteo).
